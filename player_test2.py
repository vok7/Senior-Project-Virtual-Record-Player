...
def play_media(media_uri, is_playlist=False):
    """Starts playback of the given Spotify URI on the specified device."""
    try:
        if is_playlist:
            sp.start_playback(device_id=DEVICE_ID, context_uri=media_uri)
            print(f"Now playing playlist: {media_uri}")
        else:
            sp.start_playback(device_id=DEVICE_ID, uris=[media_uri])
            print(f"Now playing track: {media_uri}")
            ...
            if status == MIFAREReader.MI_OK:
                ...
                if card_id in RFID_TO_SPOTIFY:
                    ...
                    if 'track_uri' in media_info:
                        play_media(media_info['track_uri'])
                    elif 'playlist_uri' in media_info:
                        play_media(media_info['playlist_uri'], is_playlist=True)
                    else:
                        print("No playable media found for this card.")
